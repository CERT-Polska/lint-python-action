...
LintActionStep = TypedDict(
    "LintActionStep",
    {
        "uses": str,
        "with": LintActionConfig
    }
)
...
def discover_action() -> LintActionStep:
    ...
    if not workflows_path.exists():
        raise WorkflowDiscoverError("Can't find .github/workflows. Are you in project root?")
            ...
            with yml_file.open("r") as f:
                ...
                for job, job_spec in workflow_spec["jobs"].items():
                    for step in job_spec.get("steps", []):
                        if step.get("uses").startswith("CERT-Polska/lint-python-action"):
                            return step
        except Exception:
            logging.warning(f"Unable to parse {str(yml_file)}.")
    raise WorkflowDiscoverError("Can't find lint-python-action workflow. Is it used by project?")
...
def perform_linting(action: LintActionConfig, check_only=True):
    def run_tool(command, *args, check=False):
        ...
        run_command(["python3", "-m", command, *args, *(["--check"] if check else []), action["source"]])
...
def perform_pip_install(action: LintActionConfig, with_extra: bool):
    ...
    if with_extra:
        requirements += action.get("extra-requirements", "").split()
...
def main():
    ...
    parser.add_argument("--check", action="store_true", help="Perform only check and don't modify the files")
    parser.add_argument("--install", action="store_true", help="Install required linters")
    parser.add_argument("--no-extras", action="store_false", dest="with_extras",
    ...
    try:
        ...
        logging.exception(f"Command {e.args} failed")
    except Exception:
        logging.exception("Unexpected problem found")
